...
namespace global
{
    ...
    public:
        ...
        // param eta_: the termination condition for gradient descent
        // param zeta_: the step size multiplier for gradient descent
        PotentialField(const std::vector<Obstacle> & obstacles_, const double & eta_, const double & zeta_);
        ...
        // \returns: Vector2D containing gradient in x and y dimensions
        Vector2D RepulsiveGradient(const Vector2D & cur_pos, std::vector<Obstacle> & obs);
        ...
        Vector2D FindClosestPoint(const Vector2D & cur_pos, std::vector<Vertex> vertices);
        ...
        Vector2D OneStepGD(const Vector2D & cur_pos, const Vector2D & goal);
